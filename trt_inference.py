...
img = cv2.imread("images/Test_0.jpg")
...
if __name__ == "__main__":
    ...
    batch_size = 4
    trt_engine_path = "classifier.engine"
    model = TrtModel(trt_engine_path)
    shape = model.engine.get_binding_shape(0)
    img_list = np.array((4, -1))

    for i in os.listdir("images"):
        img = cv2.imread(f"images/{i}")
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = cv2.resize(img, (224, 224))
        np.concatenate(img_list,img/255.0) # Normalize the image

    result = model(np.array(img_list),batch_size)
    print(result)'''
